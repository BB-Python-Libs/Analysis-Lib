...
# Aufgabe 1.1 a):
fexpr = sp.sqrt(1 - x**2 - y**2)
plot_scalar_field_contour(
    fexpr, x, y, x_range=(-1, 1), y_range=(-1, 1),
    title="Niveaumengen der Halbkugel: $f(x,y) = \\sqrt{1-(x^2 + y^2)}$"
)

plot_scalar_field_3d(
    fexpr, x, y, x_range=(-1, 1), y_range=(-1, 1), resolution=800,
    title="Funktionsplot der Halbkugel: $f(x,y) = \\sqrt{1-(x^2 + y^2)}$"
)

# Aufgabe 1.1 b):
fexpr = sp.sqrt(x**2 + y**2)
plot_scalar_field_contour(
    fexpr, x, y, x_range=(-1, 1), y_range=(-1, 1),
    title="Niveaumengen der Halbkugel: $f(x,y) = \\sqrt{x^2 + y^2}$"
)

plot_scalar_field_3d(
    fexpr, x, y, x_range=(-1, 1), y_range=(-1, 1), resolution=800,
    title="Funktionsplot der Halbkugel: $f(x,y) = \\sqrt{x^2 + y^2}$"
)
...
plot_scalar_field_contour(
    fexpr, x, y, x_range=(-1, 1), y_range=(-1, 1),
    title="Niveaumengen der Halbkugel: $f(x,y) = x^2 y$"
)

plot_scalar_field_3d(
    fexpr, x, y, x_range=(-1, 1), y_range=(-1, 1), resolution=800,
    title="Funktionsplot der Halbkugel: $f(x,y) = x^2 y$"
)
...
plot_scalar_field_contour(
    fexpr, x, y, x_range=(-1, 1), y_range=(-1, 1),
    title="Niveaumengen der Halbkugel: $f(x,y) = x^2 - y^2$"
)

plot_scalar_field_3d(
    fexpr, x, y, x_range=(-1, 1), y_range=(-1, 1), resolution=800,
    title="Funktionsplot der Halbkugel: $f(x,y) = x^2 - y^2$"
)

# Aufgabe 1.1 e):
fexpr = x**2 - 2*x*y**2 + x*y**3 - 1
plot_scalar_field_contour(
    fexpr, x, y, x_range=(-1, 1), y_range=(-1, 1),
    title="Niveaumengen der Halbkugel: $f(x,y) = x^2 - 2xy^2 + xy^3 - 1$"
)

plot_scalar_field_3d(
    fexpr, x, y, x_range=(-1, 1), y_range=(-1, 1), resolution=800,
    title="Funktionsplot der Halbkugel: $f(x,y) = x^2 - 2xy^2 + xy^3 - 1$"
)

# Aufgabe 1.1 f):
fexpr = (x**2 + y**2)/(x**2 + y**2 - 1)
plot_scalar_field_contour(
    fexpr, x, y, x_range=(-1/2, 1/2), y_range=(-1/2, 1/2),
    title="Niveaumengen der Halbkugel: $f(x,y) = \\frac{x^2 + y^2}{x^2 + y^2 - 1}$"
)

plot_scalar_field_3d(
    fexpr, x, y, x_range=(-0.6, 0.6), y_range=(-0.6, 0.6), resolution=800,
    title="Funktionsplot der Halbkugel: $f(x,y) = \\frac{x^2 + y^2}{x^2 + y^2 - 1}$"
)